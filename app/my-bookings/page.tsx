...
function CancelDialog({ booking, onConfirm, onClose, loading }: {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 11 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#0f172a' }}>Cancel Booking?</div>
            <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>This action cannot be undone</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: 3 }}><X size={16} /></button>
        </div>
        <div style={{ padding: '16px 22px' }}>
          <div style={{ background: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: 11, padding: '12px 14px', display: 'flex', gap: 11, alignItems: 'center', marginBottom: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, fontSize: 15, color: '#0f172a', fontFamily: 'monospace', marginBottom: 1 }}>{booking.seat?.seat_number}</div>
              <div style={{ fontSize: 12, color: '#64748b' }}>{sec?.label}</div>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 11px', background: '#f8fafc', borderRadius: 9, border: '1px solid #e2e8f0', fontSize: 13, color: '#475569' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 11px', background: '#f8fafc', borderRadius: 9, border: '1px solid #e2e8f0', fontSize: 13, color: '#475569' }}>
              <Clock size={13} color="#3b82f6" />{booking.start_time.slice(0,5)} → {booking.end_time.slice(0,5)} <span style={{ color: '#94a3b8' }}>({dur})</span>
            </div>
          </div>
          <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 12, lineHeight: 1.5 }}>Cancelling will release this seat for others to book.</p>
        </div>
        <div style={{ padding: '0 22px 18px', display: 'flex', gap: 9, justifyContent: 'flex-end' }}>
          <button onClick={onClose} disabled={loading} style={{ padding: '8px 18px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', color: '#475569', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Keep Booking</button>
...
export default function MyBookingsPage() {
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '22px 24px 0' }}>
        <div style={{ maxWidth: 940, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14, marginBottom: 18 }}>
            <div>
              <h1 style={{ fontSize: 24, fontWeight: 800, color: '#0f172a', marginBottom: 3, display: 'flex', alignItems: 'center', gap: 9 }}><ClipboardList size={21} color="#2563eb" /> My Bookings</h1>
              <p style={{ color: '#64748b', fontSize: 13 }}>Manage your workspace reservations</p>
            </div>
      ...
      <div style={{ maxWidth: 940, margin: '0 auto', padding: '18px 24px' }}>
        {loading ? (
          [1,2,3].map(i => <div key={i} style={{ height: 88, borderRadius: 13, background: '#f1f5f9', marginBottom: 9 }} />)
        ) : shown.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '56px 24px', background: '#fff', borderRadius: 15, border: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 42, marginBottom: 12 }}>{filter==='upcoming'?'📅':'📋'}</div>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 7, color: '#0f172a' }}>{filter==='upcoming'?'No upcoming bookings':'No bookings found'}</div>
            <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 20 }}>Book a seat to get started.</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            {shown.map(b => {
              ...
              return (
                <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 13, background: '#fff', border: `1.5px solid ${b.status==='cancelled'?'#f1f5f9':'#e2e8f0'}`, borderRadius: 13, padding: '14px 18px', opacity: b.status==='cancelled'?0.65:1, boxShadow: b.status==='active'?'0 1px 4px rgba(0,0,0,0.04)':'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 4 }}>
                      <span style={{ fontWeight: 800, fontSize: 14, color: '#0f172a', fontFamily: 'monospace' }}>{b.seat?.seat_number}</span>
                      ...
                      {b.seat?.machine_number != null && <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 99, background: '#f8fafc', color: '#64748b', border: '1px solid #e2e8f0' }}>Machine #{b.seat.machine_number}</span>}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', fontSize: 12, color: '#64748b' }}>
